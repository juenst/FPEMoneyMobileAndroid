...
public class MainButtonWidget extends LinearLayout {
    ...
    public static final String TAG = MainButtonWidget.class.getSimpleName();
     ...
     */
    public MainButtonWidget(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mContext = context;

        TypedArray attrsArray = context.obtainStyledAttributes(attrs, R.styleable.FinpayMoneyAttrs, 0, 0);
    ...
    private void initAttributesArray(TypedArray attrsArray) {
        ...
        mDefaultBackgroundColor = attrsArray.getColor(R.styleable.FinpayMoneyAttrs_fm_defaultColor, mDefaultBackgroundColor);
        mFocusBackgroundColor = attrsArray.getColor(R.styleable.FinpayMoneyAttrs_fm_focusColor, mFocusBackgroundColor);
        mDisabledBackgroundColor = attrsArray.getColor(R.styleable.FinpayMoneyAttrs_fm_disabledColor, mDisabledBackgroundColor);

        mEnabled = attrsArray.getBoolean(R.styleable.FinpayMoneyAttrs_android_enabled, true);

        mDisabledTextColor = attrsArray.getColor(R.styleable.FinpayMoneyAttrs_fm_disabledTextColor, mDisabledTextColor);
        mDisabledBorderColor = attrsArray.getColor(R.styleable.FinpayMoneyAttrs_fm_disabledBorderColor, mDisabledBorderColor);
        mDefaultTextColor = attrsArray.getColor(R.styleable.FinpayMoneyAttrs_fm_textColor, mDefaultTextColor);
        // if default color is set then the icon's color is the same (the default for icon's color)
        mDefaultIconColor = attrsArray.getColor(R.styleable.FinpayMoneyAttrs_fm_iconColor, mDefaultTextColor);

        mDefaultTextSize = (int) attrsArray.getDimension(R.styleable.FinpayMoneyAttrs_fm_textSize, mDefaultTextSize);
        mDefaultTextSize = (int) attrsArray.getDimension(R.styleable.FinpayMoneyAttrs_fm_textSize, mDefaultTextSize);

        mDefaultTextGravity = attrsArray.getInt(R.styleable.FinpayMoneyAttrs_fm_textGravity, mDefaultTextGravity);

        mBorderColor = attrsArray.getColor(R.styleable.FinpayMoneyAttrs_fm_borderColor, mBorderColor);
        mBorderWidth = (int) attrsArray.getDimension(R.styleable.FinpayMoneyAttrs_fm_borderWidth, mBorderWidth);

        mRadius = (int) attrsArray.getDimension(R.styleable.FinpayMoneyAttrs_fm_radius, mRadius);
        mFontIconSize = (int) attrsArray.getDimension(R.styleable.FinpayMoneyAttrs_fm_fontIconSize, mFontIconSize);

        mIconPaddingLeft = (int) attrsArray.getDimension(R.styleable.FinpayMoneyAttrs_fm_iconPaddingLeft, mIconPaddingLeft);
        mIconPaddingRight = (int) attrsArray.getDimension(R.styleable.FinpayMoneyAttrs_fm_iconPaddingRight, mIconPaddingRight);
        mIconPaddingTop = (int) attrsArray.getDimension(R.styleable.FinpayMoneyAttrs_fm_iconPaddingTop, mIconPaddingTop);
        mIconPaddingBottom = (int) attrsArray.getDimension(R.styleable.FinpayMoneyAttrs_fm_iconPaddingBottom, mIconPaddingBottom);

        mTextAllCaps = attrsArray.getBoolean(R.styleable.FinpayMoneyAttrs_fm_textAllCaps, false);
        mTextAllCaps = attrsArray.getBoolean(R.styleable.FinpayMoneyAttrs_android_textAllCaps, false);

        mGhost = attrsArray.getBoolean(R.styleable.FinpayMoneyAttrs_fm_ghost, mGhost);
        mUseSystemFont = attrsArray.getBoolean(R.styleable.FinpayMoneyAttrs_fm_useSystemFont, mUseSystemFont);

        String text = attrsArray.getString(R.styleable.FinpayMoneyAttrs_fm_text);

        if (text == null) { //no fb_text attribute
            text = attrsArray.getString(R.styleable.FinpayMoneyAttrs_android_text);
        }

        mIconPosition = attrsArray.getInt(R.styleable.FinpayMoneyAttrs_fm_iconPosition, mIconPosition);

        String fontIcon = attrsArray.getString(R.styleable.FinpayMoneyAttrs_fm_fontIconResource);

        String iconFontFamily = attrsArray.getString(R.styleable.FinpayMoneyAttrs_fm_iconFont);
        String textFontFamily = attrsArray.getString(R.styleable.FinpayMoneyAttrs_fm_textFont);

        Drawable icon = null;
        try {
            mIconResource = attrsArray.getDrawable(R.styleable.FinpayMoneyAttrs_fm_iconResource);
