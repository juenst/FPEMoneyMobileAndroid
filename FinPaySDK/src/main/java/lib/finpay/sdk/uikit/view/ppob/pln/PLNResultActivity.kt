...
class PLNResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        txtDaya.text = result?.bit61Parse?.subscriberSegmen+" / "+result?.bit61Parse?.power+"VA"
        ...
        btnConfirm.setOnClickListener {
            ...
            if(productCode == ProductCode.PLN_PREPAID) {
                DialogUtils.showDialogConfirmPayment(
                    this,
                    transNumber!!,
                    "Pembelian",
                    "Pembelian Token Listrik ${TextUtils.formatRupiah(result?.tagihan!!.toDouble())}",
                    ...
                    TextUtils.clearFormat(fee),
                    {
                        ...
                        FinpaySDK.authPin(
                            ...
                            ProductCode.PLN_PREPAID, {
                                ...
                                intent.putExtra("transactionType", "Pembelian Token Listrik ${TextUtils.formatRupiah(result?.tagihan!!.toDouble())}")
                ...
                DialogUtils.showDialogConfirmPayment(
                    ...
                    transNumber!!,
                    "Transaksi",
                    "Tagihan Listrik ${TextUtils.formatRupiah(result?.tagihan!!.toDouble())}",
                    TextUtils.clearFormat(saldo),
                    TextUtils.clearFormat(result?.tagihan!!.toString()),
                    TextUtils.clearFormat(fee),
                    {
                        ...
                        FinpaySDK.authPin(
                            ...
                            ProductCode.PLN_POSTPAID, {
                                ...
                                intent.putExtra("pinResult", it)
                                intent.putExtra("result", result)
                                intent.putExtra("transactionType", "Tagihan Listrik")
                                intent.putExtra("transNumber", result?.traxId!!)
                                intent.putExtra("theme", finpayTheme)
                                startActivity(intent)
                            }, {
    ...
    fun getUserBallance() {
        ...
        FinpaySDK.getUserBallance(java.util.UUID.randomUUID().toString(), this@PLNResultActivity, {
            ...
            progressDialog.dismiss()
            DialogUtils.showDialogError(this@PLNResultActivity, "", it, finpayTheme)
        })
    }
